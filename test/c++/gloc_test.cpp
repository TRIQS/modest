...
void assert_block2_gfs_and_vec_gf_are_close(X const &x, Y const &y, double precision) {
  ...
  for (int i = 0; i < x.size1(); ++i) {
    ...
    for (int j = 0; j < x.size2(); ++j) {
      if (x.block_names()[1][j] != y[i].block_names()[j])
        TRIQS_RUNTIME_ERROR << "Block2 GFs have different block name " << x.block_names()[i][j]
                            << " != " << y[i].block_names()[j];
...
auto run_gloc_test_case(std::string filename, double const threshold, self_energy_kind_e const self_energy_kind) {
  auto compute_gloc_wrapper = [&]() {
    ...
    if (self_energy_kind == self_energy_kind_e::ZeroSigma) {
      ...
      Sigma_imp_solver =
         to_vector<block_gf<Mesh, matrix_valued>>(sort_keys_as_int(root["ref_data"]["const_Sigma_input"]));
