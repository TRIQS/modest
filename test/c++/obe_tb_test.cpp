...
TEST(obe_tb, svo_wannier90) { // NOLINT
  ...
  one_body_elements_tb obe_tb = one_body_elements_from_wannier90("../../../test/c++/ref_data/svo", spin_kind_e::NonPolarized, atomic_shells);
  ...
  triqs::bz_int::bz_int_options opt = {.k_grid_dims = {12, 12, 12}};
  ...
  auto Sigma_block2 = make_block2_gf(1, 1, Sigma); // n?, nsigma, fill with empty sigma
  ...
  auto [_, obe_dft] = one_body_elements_from_dft_converter("ref_data/SrVO3-cubic-t2g.ref.h5");
  ...
  for(auto iw : iw_mesh) {
    std::cout << gloc_tb(0,0)[iw](0,0) << " " << gloc_dft(0,0)[iw](0,0) << std::endl; 
  }
...
// should ideally have  a spin up/down example 
