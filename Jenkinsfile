def projectName = "modest" /* set to app/repo name */
  ...
  platforms["osx-$platform"] = { -> node('osx && triqs') {
    stage("osx-$platform") { timeout(time: 1, unit: 'HOURS') { ansiColor('xterm') {
      ...
      dir(buildDir) { withEnv(platformEnv[1].collect { it.replace('\$BREW', env.BREW) } + [
        ...
        sh "cmake $srcDir -DCMAKE_INSTALL_PREFIX=$installDir -DTRIQS_ROOT=$triqsDir -DUpdate_Python_Bindings=OFF -DEnable_LFS_Tests=ON"
...
\${BUILD_LOG,maxLines=60}
    """,
    to: 'oparcollet@flatironinstitute.org, hlabollita@flatironinstitute.org, jcoulter@flatironinstitute.org, nwentzell@flatironinstitute.org',
    recipientProviders: [
