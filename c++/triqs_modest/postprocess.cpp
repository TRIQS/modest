...
namespace triqs::modest {
  ...
  constexpr auto lattice_gf(auto const &mesh, auto const &obe, auto const &Proj, auto const &mu, auto const &embedding_decomp, auto const &Sigma_w,
    ...
    return [&](auto &k_idx, auto &sigma) {
      auto PSP = [&](auto &w) {
        ...
        for (auto &&[alpha, R] : enumerated_sub_slices(embedding_decomp)) {
          ...
          out(r_all, r_all) += dagger(P) * nda::matrix<dcomplex>{Sigma_w(alpha, sigma).data()(w, r_all, r_all)} * P;
        }
        return out;
      };

      auto out = gf{mesh};
      for (auto &&[n, w] : enumerate(mesh)) { out.data()(n) = trace(inverse(w + dcomplex(0.0, broadening) + mu - obe.H.H(sigma, k_idx) - PSP(n))); }
      return out;
    };
  }
  ...
  spectral_function_w projected_spectral_function(one_body_elements_on_grid const &obe_theta, downfolding_projector const &Proj, double mu,
    ...
    auto Gloc = [&](auto &k_idx, auto &sigma) {
      ...
      for (auto &&[n, w] : enumerate(mesh)) {
        out.data()(n, r_all, r_all) =
           P * inverse(w + dcomplex(0.0, broadening) + mu - obe_theta.H.H(sigma, k_idx) - PSP(n, k_idx, sigma)) * dagger(P);
      }
      return out;
    };
  ...
  spectral_function_kw spectral_function_on_high_symmetry_path(one_body_elements_on_grid const &obe, double mu,
    ...
    auto Glatt = lattice_gf(mesh, obe, obe.P, mu, embedding_decomp, Sigma_w, broadening);

    for (auto k_idx : range(n_k)) {
      for (auto sigma : range(n_sigma)) { data(sigma, k_idx, r_all) = (-1.0 / M_PI) * imag(Glatt(k_idx, sigma).data()); }
    }
