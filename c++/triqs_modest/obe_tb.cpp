...
namespace triqs::modest {
  ...
  block2_gf<Mesh, matrix_valued> gloc(one_body_elements const &obe, double mu, block2_gf<Mesh, matrix_valued> const &Sigma_dynamic,
    ...
    for (auto sigma : range(n_sigma)) {
      ...
      // spin index
      auto Sigma_big_space = gfs::gf(mesh, {obe.H[sigma].n_orbitals(), obe.H[sigma].n_orbitals()}); //
      for (auto &&[block, R] : enumerated_sub_slices(embedding_decomp)) { Sigma_big_space.data()(r_all, R, R) = Sigma_dynamic(block, sigma).data(); }
