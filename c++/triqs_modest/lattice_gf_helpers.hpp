...
namespace triqs::modest::detail {
  ...
  /// μ-independent Woodbury inputs derived from Σ.
  struct woodbury_setup {
    std::vector<dcomplex> omegas;
    active_subspace_t active;
    std::vector<nda::array<dcomplex, 3>> Sa_per_sigma; ///< Σ_active(ω) per σ: (n_ω, rank, rank)
  };

  template <typename Mesh>
  woodbury_setup make_woodbury_setup(block2_gf<Mesh, matrix_valued> const &Sigma_dynamic, nda::array<nda::matrix<dcomplex>, 2> const &Sigma_static) {
    auto Sigma_total = make_sigma_total(Sigma_dynamic, Sigma_static);
    auto const &mesh = Sigma_total(0, 0).mesh();
    auto decomp      = get_struct(Sigma_total).dims(r_all, 0) | tl::to<std::vector>();
    auto active      = detect_active_subspace(Sigma_total, decomp);
    return {.omegas       = mesh | tl::to<std::vector<dcomplex>>(),
            .active       = active,
            .Sa_per_sigma = range(Sigma_total.size2()) | stdv::transform([&](long s) { return compute_sigma_active(Sigma_total, active, s); })
               | tl::to<std::vector>()};
  }

  // Build the full band-basis Green's function G(ω) = [ω + iδ + μ − H(k) − P†Σ(ω)P]⁻¹ for a fixed (k, σ),
  // returned as (n_ω, N_ν, N_ν), via the rank-reduced Woodbury identity for a diagonal band-basis H(k).
  inline nda::array<dcomplex, 3> build_G_band(one_body_elements_on_grid const &obe, active_subspace_t const &active,
  ...
  inline nda::array<double, 2> make_uniform_k_mesh(bz_int_options const &opt) {
    if (opt.run_adaptive) {
      throw std::runtime_error(
         "make_uniform_k_mesh: adaptive integration is unsupported "
