...
namespace triqs::modest {
  ...
  namespace detail {
    ...
    // Nearest-grid-point shift maps for ω+Ω, valid for any sorted (possibly non-uniform) ω mesh:
    //   jmap(iq, n)  = index of the ω grid point nearest to ω_n + Om(iq)
    //   valid(iq, n) = 1 if ω_n + Om(iq) lies within [ω_0, ω_{N-1}], else 0
    std::pair<nda::array<long, 2>, nda::array<long, 2>> build_shift_maps(nda::array<double, 1> const &omega, nda::array<double, 1> const &Om) {
      long n  = omega.extent(0);
      long nq = Om.extent(0);
      ...
      for (long iq = 0; iq < nq; ++iq) {
        for (long i = 0; i < n; ++i) {
          double t     = omega(i) + Om(iq);
          bool in_grid = (t >= lo && t <= hi);
          valid(iq, i) = in_grid ? 1 : 0;
          if (!in_grid) {
            jmap(iq, i) = 0;
            continue;
          }
          // First grid point ≥ t, then pick the nearer of it and its predecessor (ties → lower index).
          long r      = long(std::lower_bound(omega.data(), omega.data() + n, t) - omega.data());
          jmap(iq, i) = (r > 0 && t - omega(r - 1) <= omega(r) - t) ? r - 1 : r;
        }
      }
      return {jmap, valid};
    }
    ...
    // (The caller guarantees a diagonal H(k); matrix-valued dispersions are rejected in transport_distribution.)
  ...
  transport_distribution_t transport_distribution(one_body_elements_on_grid const &obe, double mu,
    ...
    auto [jmap, valid] = detail::build_shift_maps(omega_arr, Om_mesh);
            ...
            for (long n = 0; n < n_w; ++n) {
              for (long iq = 0; iq < n_Om; ++iq) {
                ...
                for (long i = 0; i < n_ov; ++i) {
                  ...
                  for (long jj = 0; jj < n_ov; ++jj) tot += vA_a(j, i, jj) * vA_b(n, jj, i);
                }
